...
public class CR14 extends RegGIA implements Comparable<Object>{
	...
	private void checkVALORCONTABIL(){
		...
		if ((diff.abs()).compareTo(Env.ONE) <= 0){ //ATE R$1 FAZ AJUSTE DO CALCULO
			if (VALOR_CONT_1.signum() == 1)
				VALOR_CONT_1 = calcValor;
			else if (VALOR_CONT_2.signum() == 1)
				VALOR_CONT_2 = calcValor;
		}
	} //checkVALORCONTABIL
	
	public BigDecimal getVALORCONTABIL() {
		if (VALOR_CONT_1 == null)
			VALOR_CONT_1 = Env.ZERO;
		
		if (VALOR_CONT_2 == null)
			VALOR_CONT_2 = Env.ZERO;
	...
	public BigDecimal getBASECALCULO() {
		if (BASE_CALC_1 == null)
			BASE_CALC_1 = Env.ZERO;
		
		if (BASE_CALC_2 == null)
			BASE_CALC_2 = Env.ZERO;
