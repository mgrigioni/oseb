...
public class ValidatorInOut implements ModelValidator
{
		...
		for(MMovementLine line : lines) {
			...
			if(prod.contains("" + line.getM_Product_ID() + "|" + line.getM_Locator_ID())){
				msg = "Duas linhas usando o mesmo produto na mesma posição. " +
					  "(" + line.getM_Product().getValue() + "|" + line.getM_Locator().getValue()+")";
				break;
			}
			else{
			...
			if(qtdOnHand.compareTo(line.getMovementQty()) == -1){
				msg = "Sem saldo na linha=" + line.getLine() + 
					  " (" + line.getM_Product().getValue() + "|" + line.getM_Locator().getValue() + "|" + line.getMovementQty() + ")";
				break;
			}
			
			if (line.getMovementQty().signum() == -1){
				qtdOnHand = getQtyOnHand(line.getM_Product_ID(), line.getM_LocatorTo_ID(), line.getM_AttributeSetInstance_ID());
				if (qtdOnHand.compareTo(line.getMovementQty().abs()) == -1){
					msg = "Sem saldo na linha=" + line.getLine();
					break;
				}
			}
		...
		if (beforeComplete)
		{
			...
			for (int i = 0; i < lines.length; i++)
			{
				...
				if (M_Product_ID == 0 && C_Charge_ID == 0){
					...
					//continue;
					return "Linha " + line.getLine() + " sem produto/despesa";
				}
	...
	{
		StringBuffer sb = new StringBuffer ("ValidatorInOut@AdempiereLBR");
		return sb.toString ();
	}	//	toString
